...
```
I get slightly different values for both models. The simple linear regression model now has an RMSE of 696.8889145 and a standard error of 72.62528985, which shows more variance in the model than the previous time I ran it. The multiple linear regression model has an RMSE of 631.1949075 this time, and the standard error is 70.40354955. The conclusion about which model performs better remains the same: the multiple linear regression out-performs the simple linear regression. The pattern of the multiple linear regression model having a lower standard error and less variability also remains.
