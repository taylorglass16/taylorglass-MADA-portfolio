...
```
The RMSE for the simple linear model decreased to 686.885, which is substantially smaller than 702.8078. The cross-validation method of testing the model shows that it performs better than the train/test splitting metho did. The standard error for the RMSE is 69.71588 when using the cross-validation method, which allows us to observe the variability in RMSE estimates. 
...
# initialize a workflow and fit it to the training data for the null model
mavo_wf3 <- workflow() %>%
  add_model(null_model) %>% 
  add_formula(Y~ 1) %>% #specify the formula since we are not using a recipe
  fit(data = train_data) 
...
collect_metrics(mavo_wf_rs2)
```
Nothing changes for the null model, which is to be expected. 
